...
public class Metadata {
    ...
    /**
     * Activates a state for this Metadata. If the state is not in the Hashmap
    ...
    void deactivate(State state) {
        if (activeStates.containsKey(state)) {
            ...
            if (state instanceof PseudoState
                    && (((PseudoState) state).type == PseudoState.pseudostate_deep_history || ((PseudoState) state).type == PseudoState.pseudostate_history)) {
                data.active = false;
                return;
            }
     ...
     * method is, that a new StateRuntimedata is created if it don't exists in
     * the hashmap. If the data already exists, it is returned instead.
     */
