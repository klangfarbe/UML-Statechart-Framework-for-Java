...
public class SemanticTests {
    ...
    public void testSemantics7() throws StatechartException {
        ...
        Assert.assertTrue(chart.start(data, parameter));
        Assert.assertEquals("D:start A:a D:a A:j1 D:j1 E:a2 A:c D:c A:j2 D:j2 E:a3 A:j3 D:j3 E:a4 A:end", parameter.path);
        chart.shutdown();
    }
    ...
    public void testSemantics8() throws StatechartException {
        ...
        Assert.assertTrue(chart.start(data, parameter));
        Assert.assertEquals("D:start A:p A:start p D:start p A:a D:a A:b D:b A:end p D:end p D:p A:end", parameter.path);
        chart.shutdown();
    }
    ...
    public void testSemantics11() throws StatechartException {
        ...
        Assert.assertTrue(chart.start(data, parameter));
        Assert.assertTrue(chart.dispatch(data, event, parameter));
        Assert.assertEquals("D:start A:p A:start p D:start p A:a D:a D:p A:end", parameter.path);
        chart.shutdown();
    }
    ...
    public void testSemantics14() throws StatechartException {
        ...
        Assert.assertTrue(chart.start(data, parameter));
        Assert.assertEquals("D:start A:p A:q A:r D:r D:q D:p A:x A:y D:y D:x A:end", parameter.path);
        chart.shutdown();
    }
    ...
    // Checks the deep history state when the transition is made from the
    // substate
    // Be aware that this does not work with a correct semantics but we make
    // sure
    ...
    public void testSemantics30() throws StatechartException {
        ...
        String result = "D:start A:p " + "A:p-r1 A:start p-r1 D:start p-r1 A:a " + "A:p-r2 A:start p-r2 D:start p-r2 A:b " + "D:b E:t2 A:end p-r2 "
                + "D:a E:t1 A:end p-r1 " + "D:end p-r1 D:p-r1 " + "D:end p-r2 D:p-r2 " + "D:p " + "A:end";
